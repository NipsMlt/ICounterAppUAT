...
class NativeUsb extends Connection{
    private final String TAG = "NativeUsb";
    ...
    protected int bulkWrite(byte[] data, int timeout) {
        try {
            if (mUsbConnection.claimInterface(mUsbInterface, true)) {
                return mUsbConnection.bulkTransfer(mUsbOutEndpoint, data, data.length, timeout);
            } else {
                mOpened = false;
                return -1;
            }
    ...
    protected int bulkRead(byte[] data, int dataLength, int timeout) {
        try{
            if (mUsbConnection.claimInterface(mUsbInterface, true)) {
                return mUsbConnection.bulkTransfer(mUsbInEndpoint, data, dataLength, timeout);
            }
            else {
                mOpened = false;
