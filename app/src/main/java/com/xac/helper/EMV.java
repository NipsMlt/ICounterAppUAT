...
public class EMV extends VNG {
    ...
    public byte [] getCmdBuffer()
    {
        if (dataField != null) {
            ...
            addData(data);
            buffer[3] = (byte)(((data.length + 6) >> 8) & 0xFF);
            buffer[4] = (byte)((data.length + 6) & 0xFF);
            buffer[9] = (byte)((data.length >> 8) & 0xFF);
            buffer[10] = (byte)(data.length & 0xFF);
        }
        return super.getCmdBuffer();
    }

    public boolean tryToParse() {
        if (cmdSize < 11)   // minimum package size
            return false;
        ...
        if (dataFieldLen + parseIndex > cmdSize)
            return false;
    ...
    public boolean checkError() {
        if (cmdId == CMD_ID._51_PayStart) {
            ...
            if (parameter[0] == (byte)0) {
                if (parameter[1] == 0x00 || parameter[1] == 0x01 || parameter[1] == 0xC8 || parameter[1] == 0xC9)
                    return true;
            }
        } else {
            if (parameter[0] == (byte)0)
    ...
    public static class ICS_DATA {
        ...
        public byte byCardDataInput;    // bit8: Manual Key Entry
    ...
    public static class EMV_MAP {
        ...
        static
        {
            ...
            ERR.put( 0xF503, "EMVERR_STOP_TRANSACTION" );
    ...
    public static class APP_CONFIG {
        ...
        /**
         * QZ2 <br/>
         * 4 bytes = {Key#}{Padding}{Hash}{Sentinel} <br/>
         * Key#: 1 ASCII byte, key index for encrypting account data <br/>
