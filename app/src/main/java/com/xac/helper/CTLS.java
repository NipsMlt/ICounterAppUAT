...
public class CTLS extends VNG
{
    ...
    public byte [] getCmdBuffer() {
        ...
        buffer[3] = (byte)(((cmdSize - 5) >> 8) & 0xFF);
        buffer[4] = (byte)((cmdSize - 5) & 0xFF);
        buffer[6] = (byte)(((cmdSize - 8) >> 8) & 0xFF);
        buffer[7] = (byte)((cmdSize - 8) & 0xFF);
    ...
    public boolean tryToParse() {
        ...
        if (cmdSize < 9)   // minimum package size
            return false;
        ...
        if (cmd.equals("QN") || cmd.equals("QR")) {
            ...
            ins = parseByte();
            status = parseByte();
            int ctlsDataLen = parseValue(2);

            if (ctlsDataLen + parseIndex > cmdSize)
                return false;
    ...
    public String parseCtlsData(TLV e2) {
        ...
        {
            if (ctlsData[index] == (byte) VNG.FS)
            {
                j++;
            }
            else if (ctlsData[index] == (byte) VNG.RS)
            {
                index++;
                break;
            }
